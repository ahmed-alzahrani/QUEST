...
public class deckBuilder
{
    ...
    public List<Card> generateEventCards()
    {
        ...
        eventCards.Add(new EventCard("Event Card", "Chivalrous Deed", "Player(s) with both lowest rank and least amount of shields, receives 3 shields", new iStoryChivalrousDeed()));
        eventCards.Add(new EventCard("Event Card", "Pox", "All players except the player who drew this card lose 1 shield", new iStoryPox()));
        eventCards.Add(new EventCard("Event Card", "Plague", "Drawer loses 2 shields if possible", new iStoryPlague()));
        eventCards.Add(new EventCard("Event Card", "King's Recognition", "The next player(s) to complete a Quest will receive 2 extra shields", new iStoryKingsRecognition()));
        eventCards.Add(new EventCard("Event Card", "King's Recognition", "The next player(s) to complete a Quest will receive 2 extra shields", new iStoryKingsRecognition()));
        eventCards.Add(new EventCard("Event Card", "Queen's Favor", "The lowest ranked player(s) immediately receives 2 Adventure Cards", new iStoryQueensFavor()));
        eventCards.Add(new EventCard("Event Card", "Queen's Favor", "The lowest ranked player(s) immediately receives 2 Adventure Cards", new iStoryQueensFavor()));
        eventCards.Add(new EventCard("Event Card", "Court Called to Camelot", "All Allies in play must be discarded", new iStoryCourtCalled()));
        eventCards.Add(new EventCard("Event Card", "Court Called to Camelot", "All Allies in play must be discarded", new iStoryCourtCalled()));
        eventCards.Add(new EventCard("Event Card", "King's Call to Arms", "The highest ranked player(s) must place 1 weapon in the discard pile. If unable to do so, 2 Foe Cards must be discarded", new iStoryKingsCallToArms()));
        eventCards.Add(new EventCard("Event Card", "Prosperity throughout the Realm", "All players may immediately draw 2 Adventure Cards", new iStoryProsperity()));
        return eventCards;
    }
